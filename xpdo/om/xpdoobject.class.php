                    ...
                    switch ($fieldType) {
                        ...
                        case 'timestamp' :
                            if ($dbType == 'int' || $dbType == 'integer' || $dbType == 'INT' || $dbType == 'INTEGER') {
                                $ts= intval($value);
                                $value = $ts ? strftime('%Y-%m-%d %H:%M:%S', $ts) : '0000-00-00 00:00:00';
                            }
                            elseif (in_array($value, $this->_currentTimestamps)) {
                                $value= strftime('%Y-%m-%d %H:%M:%S');
                            }
                            if ($value && is_string($format) && !empty ($format)) {
                                if (strpos($format, 're:') === 0) {
                                    if ($value= date('Y-m-d H:M:S', $value) && !empty ($formatTemplate) && is_string($formatTemplate)) {
                                        $value= preg_replace(substr($format, 3), $formatTemplate, $value);
                                    }
                        ...
                        case 'datetime' :
                            ...
                            elseif (is_string($format) && !empty ($format)) {
                                if (strpos($format, 're:') === 0) {
                                    ...
                                    $value= date($format, $value);
                                } elseif ($ts= strtotime($value) != -1 && $ts !== false) {
